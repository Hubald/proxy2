...
class ProxyRequestHandler(BaseHTTPRequestHandler):
    ...
    def connect_intercept(self):
        ...
        confpath = "{}/{}.cnf".format(self.certdir.rstrip('/'), hostname)
        ...
        with self.lock:
            if not os.path.isfile(certpath):
                ...
                p1 = Popen(["openssl", "req", "-new", "-key", self.certkey, "-subj", "/CN={}".format(hostname), '-sha256'], stdout=PIPE)
                p2 = Popen(["openssl", "x509", "-req", "-extfile", confpath, "-days", "3650", "-CA", self.cacert, "-CAkey", self.cakey, "-set_serial", epoch, "-sha256", "-out", certpath], stdin=p1.stdout, stderr=PIPE)
                p2.communicate()
                os.unlink(confpath)
